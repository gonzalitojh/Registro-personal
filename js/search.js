...
// Iteración 3: se amplía `cast` a 10 (feedback: 5 dejaba fuera reparto
// secundario buscable; 10 sigue siendo <1KB por documento y cubre el
// bloque principal de TMDB) y se normaliza el guard de director.
export function minimalStoredFields(details, type) {
  ...
  if (type === "movie") {
    ...
    if (details.cast && Array.isArray(details.cast) && details.cast.length) keep.cast = details.cast.slice(0, 10);
