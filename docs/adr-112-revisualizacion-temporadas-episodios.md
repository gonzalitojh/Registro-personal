...
### 9. Iteración 7 (feedback 2026-08-20)

Séptima ronda de feedback del usuario en la issue #310 (comentario
2026-08-20T10:42:08Z): «Aunque un episodio no esté marcado, si tiene
valoración o visualizaciones previas, estas deben mostrarse, en lugar
de mostrarse solo cuando se marca el episodio.»

**Causa raíz**: `applyEpisodeRowState` y `renderEpisodeRows` derivaban la
visibilidad del bloque de meta (estrellas + «Visionados anteriores») de
`checked` (el episodio marcado en el ciclo actual): un episodio con
visionados históricos o valoración previa que quedaba **sin marcar**
durante un rewatch (por el criterio por contador de la iteración 5)
ocultaba sus datos, aunque existieran.

**Decisión: los datos se muestran por DATOS REALES, no por estado**
(`js/ui.js`), con la semántica del patrón #136 (el estado visual siempre
deriva de los datos, nunca al revés):
- `times` deja de ser `checked ? Math.max(1, times) : 0` y pasa a ser el
  **contador real de la entrada** (`entry ? Math.max(1, Number(entry.times) || 1) : 0`):
  un episodio marcado 4 veces fuera del ciclo actual muestra «4» igual
  que cuando estaba marcado.
- Nuevo discriminador `hasData = Boolean(entry && (times > 0 || entry.rating != null))`:
  el bloque de meta, el botón «Visionados anteriores» y el listado de
  fechas se muestran cuando hay **cualquier** dato (visualizaciones o
  valoración), esté el episodio marcado o no; solo se ocultan cuando el
  episodio no se ha visto nunca (sin entrada).
- El `aria-label` del checkbox distingue ahora tres casos: marcado en el
  ciclo (con su texto de siempre), sin marcar pero con datos («visto N
  veces… Pulsa para verlo de nuevo») y sin datos («Marcar … como visto»).
- El circuito de la casilla (`data-count`) ya mostraba el contador
  independiente del marcado; con `times` real ahora es consistente con
  el botón.
- `ocio/ocio.css` (regla 4 de AGENTS.md): el contador del círculo se
  muestra también sobre la casilla **sin marcar** (antes solo con
  `:checked`), con `color: var(--ink)` en la base; el marcado mantiene
  `--paper`; en negro puro la casilla sin marcar es una superficie
  oscura (`--white ≈ #0f0f0f`) y el número pasa a `--paper`, mientras
  que la marcada usa teal claro (`--teal-reel-dark ≈ #4f9c8e`) y el
  número a `--ink` (patrón luz-sobre-oscura / oscuro-sobre-clara del
  resto de overrides agrupados; ≈6.5:1 y ≈7:1 AA respectivamente).
- Versión PWA bumped a `20261010`.
...
- **Iteración 5**: los ciclos de rewatch iniciados **con la versión
  ...
  `20261009` y la iteración 7 (feedback 2026-08-20) a `20261010`.
- **Iteración 7**: el bloque de meta de la fila de episodio (estrellas +
  «Visionados anteriores») se muestra con datos reales aunque el
  episodio no esté marcado en el ciclo; solo se oculta cuando el
  episodio no se ha visto nunca. El contador del círculo de la casilla
  se lee también sin marcar en los cuatro modos (overrides agrupados de
  negro puro).
