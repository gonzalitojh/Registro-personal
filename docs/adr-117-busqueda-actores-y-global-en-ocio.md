...
- Buscar «Tom Hanks», «Bryan Cranston» o «Vince Gilligan» encuentra las películas/series donde participan, incluso con coincidencias parciales e insensibles a mayúsculas; `cast` como objetos `{name}` ya se indexa correctamente y fichas antiguas sin reparto lo obtienen al abrir la ficha (needsDetailFetch ampliado).
- Los nuevos títulos guardan `cast`/`director`/`creators`; los antiguos los obtienen al abrir la ficha una vez (revalidación bajo demanda, ahora también sin necesidad de que falte `overview`). No se requiere migración masiva.
...
- La PWA invalida cachés anteriores al subir la versión (20261020 tras el segundo fix).
