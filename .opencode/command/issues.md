...
1. Ejecuta el helper del proyecto con el filtro indicado:
   ...
   - `--todo`: issues listas para que el agente las aborde (ai-todo).
   - `--review`: issues con PR pendiente de aprobación (ai-needs-review).
   - `--blocked`: issues bloqueadas (ai-blocked).
   - `--done`: issues completadas (ai-done), incluidas las cerradas.
...
4. Destaca cuántas issues están listas para que el agente las aborde (ai-todo).
